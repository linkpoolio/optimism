...
Alternatively, you can re-issue `admin_setSdmPostExecOptIn` through the RPC interface after each restart of those processes.
</Info>
...
To check your client version, use the `--version` switch.  E.g.
...
Set the environment variable `OP_SDM_POLICY` to the desired SDM policy in your shell.  All three components are aware and will detect this environment variable.
```bash
...
Each of the three components also reads the switch  `--sdm-policy` on launch, .e.g.
```bash
./op-node --sdm_policy=BlockLevelWarming
```
...
If the component is already running, you can call the RPC method `admin_setSdmPolicy` with the argument `BlockLevelWarming` on each component's RPC endpoint, e.g.
...
You can verify the current configuration for each component with the method `admin_sdmStatus` with no parameter e.g.
...
The response reports the `effective` value, which defines if SDM is currently active.  It is the logical `AND` of the two gates: the protocol gate, `protocolActive`, set by the **Lagoon** hardfork and the chain operator gate, `postExecOptIn`, set by you, e.g.:
...
regular transactions.  Then your **rollup-boost** instance materializes it into the final sealed block. You still need to opt **op-rbuilder** in via one of the three methods above, while any SDM aware version of **rollup-boost** detects and assembles the final PostExec automatically.
