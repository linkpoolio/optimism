...
The rebates are decided by the execution engine and recorded in a special
**PostExec transaction** appended to every SDM-active block where the sequencer
issues rebates. Verifiers re-derive the transactions and verify their
discounted gas fees against the PostExec transaction. SDM is therefore a
...
To persist it, the chain operator can choose between either exporting the
environment variable `OP_SDM_POLICY=BlockLevelWarming` or passing the CLI switch
`--sdm-policy=BlockLevelWarming`.
Alternatively, you can re-issue `admin_setSdmPostExecOptIn` after each restart of those
processes.
</Info>
